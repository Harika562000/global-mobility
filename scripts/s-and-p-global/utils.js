 ...
 * Actual Solr field name is resolved at runtime from /tag-config.json
 * via loadTagConfigKeyToField().
...
async function groupTagPathsByPathPrefix(tagPathsInput) {
  ...
  paths.forEach((fullPath) => {
    ...
    const match = LUCIDWORKS_PATH_PREFIX_TO_CONFIG_KEY
      .find(([prefix]) => restLower.startsWith(prefix));
    if (match) {

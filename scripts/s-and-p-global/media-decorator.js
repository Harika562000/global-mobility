 ...
 */
const AEM_PUBLISH_HOST = 'publish-p184787-e1941710.adobeaemcloud.com';
 ...
 */
export async function resolveVideoSrc(href) {
  if (!href) return href;
  try {
    ...
    if (url.pathname.startsWith('/content/dam/')) {
      ...
      if (url.hostname.startsWith('author-')) {
        ...
        url.hostname = await getConfigValue('AEM_PUBLISH_HOST') || AEM_PUBLISH_HOST;
        url.port = '';
      }
      return url.toString();
    }
...
export async function decorateMedia(container, variant = 'default', overrides = {}) {
  ...
  await Promise.all(videoLinks.map(async (link) => {
    const videoSrc = await resolveVideoSrc(link.href);

...
function decorateButtons(element) {
  ...
  /* Group 2+ adjacent button <p> elements into a <div class="button-container"> wrapper.
     ...
     Also mark document links (PDF, DOCX, …) with a download attribute. */
  const docExt = /\.(pdf|docx?|xlsx?|pptx?|zip|rar)$/i;
  const buttonParas = [];
  element.querySelectorAll('a.button').forEach((btn) => {
    const p = btn.closest('p');
    if (p && !p.closest('.button-container') && !buttonParas.includes(p)) {
      buttonParas.push(p);
    }
    try {
      const { pathname } = new URL(btn.href, window.location.origin);
      if (docExt.test(pathname)) {
        btn.setAttribute('download', pathname.split('/').pop());
      }
    } catch { /* invalid URL – skip */ }
  });

  let idx = 0;
  while (idx < buttonParas.length) {
    const group = [buttonParas[idx]];
    const parent = buttonParas[idx].parentElement;
    let next = idx + 1;
    while (
      next < buttonParas.length
      && buttonParas[next].parentElement === parent
      && buttonParas[next].previousElementSibling === group[group.length - 1]
    ) {
      group.push(buttonParas[next]);
      next += 1;
    }
    if (group.length >= 2) {

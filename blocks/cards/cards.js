...
export default async function decorate(block) {
  ...
  // Only target spans that don't already have an img to avoid adding duplicates
  // for icons that were decorated before the block's JS rebuilt the DOM.
  try {
    block.querySelectorAll('span.icon:not(:has(img))').forEach((span) => {
      decorateIcons(span);
    });
  } catch (e) {

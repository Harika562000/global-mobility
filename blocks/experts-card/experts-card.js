...
const CARD_IMAGE_BREAKPOINTS = [
  { media: '(width >= 1025px)', width: '1200' },
  { width: '600' },
];

const EXPERTS_CAROUSEL_SETTINGS = {
  ...DEFAULT_CAROUSEL_OPTIONS,
  /* Carousel below 1025px; initCarousel */
  mobileOnly: true,
  infinite: false,
  showBottomNav: false,
...
function isEmptyRow(row) {
  if (row.dataset?.aueModel === 'experts-card-item') return false;
...
/** Manual UE rows: image + title + description + link (4 cols), or + alt text (5 cols). */
function isExpertCardRow(row) {
  if (!row || row.tagName !== 'DIV') return false;
  if (row.dataset.aueModel === 'experts-card-item') return true;
  const n = row.children.length;
  if (n < 4) return false;
  /* Authoring: new “+” items may be 4–5 empty cells before image/link exist */
  if (isAuthorEnvironment() && n <= 5) return true;
  const first = row.children[0];
  const last = row.children[n - 1];
  const hasImage = !!first?.querySelector('picture, img');
  const hasLink = !!last?.querySelector('a[href]');
...
function extractFlatExpertsContentConfig(block) {
  const result = {
    contentSource: '',
    pageUrls: [],
    cardTags: '',
    /** Rows whose links are page picks for manual/tags source — not section CTAs. */
    urlPickerRows: new Set(),
  };
  const seen = new Set();
  ...
  [...block.children]
    ...
    .forEach((row) => {
      ...
      if (afterSource) {
        const link = cell.querySelector('a[href]');
        if (link) {
          result.urlPickerRows.add(row);
          tryPushPathname(link.getAttribute('href') || link.href || '', seen, result.pageUrls);
        }
      }
    });
...
function resolveContentSourceAndUrls(block, config, manualCards, flat = null) {
  const flatConfig = flat || extractFlatExpertsContentConfig(block);
...
async function buildCardsForSource({
  ...
  if (contentSource === 'tags') {
    const fromSearch = await fetchCardsFromTagSearch(cardTags, cardCount, {
      maxCards: MAX_CARDS,
      pageType: LUCIDWORKS_PAGE_TYPES.EXPERTS,
      blockConfig: blockConfig || {},
    });
    /* No fallback to manual cards when Tags is selected. */
    return fromSearch.map((c) => ({
      imageUrl: c.imageUrl,
      title: c.title,
...
function extractFallbackHeader(rows, urlPickerRows = new Set()) {
  const fallback = {
    eyebrow: '',
    titleHTML: '',
    ctaHref: '',
  ...
  rows.forEach((row) => {
    if (isExpertCardRow(row)) return;
    if (urlPickerRows.has(row)) return;
    const cell = getValueCell(row) || row;
    if (!cell) return;
...
function createHeader(config, cells, fallbackHeader = {}) {
  ...
  if (authoredCta) {
    ...
    container.className = 'button-container experts-card-cta-button';
...
function getManualCards(blockRows) {
  const mapped = blockRows
    .filter(isExpertCardRow)
    .map((row) => {
      /* 4 cols: image, title, description, link. 5+: image, alt, title, description, link. */
      const hasImageAltField = row.children.length >= 5;
      ...
      const linkCell = row.children[baseIdx + 2];
...
function createCardElement(card) {
  ...
  bodyRow.className = 'experts-card-body-row';

  const copy = document.createElement('div');
  copy.className = 'experts-card-copy';

  const titleEl = document.createElement('p');
  titleEl.className = 'experts-card-title';
  titleEl.textContent = card.title.split('|')[0] || '';
  copy.append(titleEl);

  if (card.description) {
    const descEl = document.createElement('p');
    descEl.className = 'experts-card-description';
    const descText = document.createElement('span');
    descText.className = 'experts-card-description-text';
  ...
  arrowWrap.className = 'experts-card-arrow';
  arrowWrap.append(createArrowIcon());

  bodyRow.append(copy, arrowWrap);
  body.append(bodyRow);
...
function wireExploreCtaPlacement(block, actions) {
  const exploreCta = actions.querySelector('.experts-card-cta-button');
  if (!exploreCta) return;

  exploreCta.classList.add('experts-card-explore-cta');
  const mobileMq = window.matchMedia('(max-width: 1025px)');
...
export default async function decorate(block) {
  ...
  const flatContent = extractFlatExpertsContentConfig(block);
  const fallbackHeader = extractFallbackHeader(rows, flatContent.urlPickerRows);
  ...
  const cardTagsForSearch = contentSource === 'tags'
    ...
    : cardTags;
  const cards = await buildCardsForSource({
    contentSource,
    pageUrls,
  ...
  if (cards.length > 1) {
    await initCarousel(cardsTrack, {
      mobileOnly: EXPERTS_CAROUSEL_SETTINGS.mobileOnly,
      infinite: EXPERTS_CAROUSEL_SETTINGS.infinite,
      showBottomNav: EXPERTS_CAROUSEL_SETTINGS.showBottomNav,
      transitionDurationMs: EXPERTS_CAROUSEL_SETTINGS.transitionDurationMs,
      transitionEasing: EXPERTS_CAROUSEL_SETTINGS.transitionEasing,
      dragThresholdPx: EXPERTS_CAROUSEL_SETTINGS.dragThresholdPx,
      swipeCommitPx: EXPERTS_CAROUSEL_SETTINGS.swipeCommitPx,
      swipeSlideFactor: EXPERTS_CAROUSEL_SETTINGS.swipeSlideFactor,
      clampToMaxOffset: EXPERTS_CAROUSEL_SETTINGS.clampToMaxOffset,
    });
  }
